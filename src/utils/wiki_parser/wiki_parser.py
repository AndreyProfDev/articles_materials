...
def store_articles_to_yml_file(target_folder: Path | None, 
                                   stage_subfolder: str | None,
                                   file_name: str | None, 
                                   content: list[SingleArticle]) -> None:
    if target_folder is not None and stage_subfolder is not None and file_name is not None:
        ...
        with open(target_folder / file_name, "w") as file:
            yaml.dump([article.model_dump() for article in content], file, allow_unicode=True)
...
def process_wiki_markdown_in_pages(articles: list[SingleArticle]) -> list[SingleArticle]:
    articles = copy.deepcopy(articles)

    for article in articles:
        for section in article.sections:
            section.content = wiki_markdown_processor.process_wiki_markdown(section.content)
            section.content = section.content.strip()
    return articles
...
def split_wiki_page_by_sections(articles: list[SingleArticle]) -> list[SingleArticle]:
    ...
    for article in articles:
        splitted_sections = []
        for section in article.sections:
            splitted_sections.extend(wiki_sections_splitter.split_wiki_text_by_sections(section.content))
...
def extract_articles_from_mediawiki_xml(wiki_xml: str, output_folder: Path | None = None, file_name: str | None = None) -> list[SingleArticle]:

    pages = extract_text_from_wiki_xml(wiki_xml)
    store_articles_to_yml_file(target_folder=output_folder, 
                                   stage_subfolder='1. extracted_pages',
                                   file_name=file_name,
                                   content=pages)

    pages = process_wiki_html(pages)
    store_articles_to_yml_file(target_folder=output_folder,
                                  stage_subfolder='2. processed_html_pages',
                                  file_name=file_name,
                                  content=pages)

    pages = process_wiki_markdown_in_pages(pages)
    store_articles_to_yml_file(target_folder=output_folder,
                                 stage_subfolder='3. processed_markdown_pages',
                                 file_name=file_name,
                                 content=pages)
    ...
    store_articles_to_yml_file(target_folder=output_folder,
                                   stage_subfolder='4. split_sections',
                                   file_name=file_name,
                                   content=articles)
    
    articles = remove_empty_articles(articles)
    store_articles_to_yml_file(target_folder=output_folder,
                                   stage_subfolder='5. remove_empty_articles',
                                   file_name=file_name,
                                   content=articles)
    return articles

def extract_articles_from_file(file_path: str, output_folder: Path | None = None) -> list[SingleArticle]:
    with open(file_path, "r") as file:
        wiki_xml = file.read()
    return extract_articles_from_mediawiki_xml(wiki_xml, output_folder = output_folder, file_name = Path(file_path).name)