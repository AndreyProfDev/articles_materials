...
from embedding_models.providers import hugging_face, open_ai
from embedding_models.schema import MODEL_PROVIDER, EmbeddingModelInfo
from metaflow import Config
from metaflow.client.core import Flow
from metaflow.decorators import step
from metaflow.flowspec import FlowSpec
from vectordb.vectordb import VectorIndex


def get_model_provider(provider: MODEL_PROVIDER, config: EmbeddingModelInfo, open_ai_key: str):
...
class WikiArticlesDataFlow(FlowSpec):
    ...
    def start(self):
        # Load data
        self.data = Flow("WikiArticlesDataFlow").latest_run.data.final_df
    ...
    def embed_articles(self):
        ...
        for record in self.input:
            record["embeddings"] = self.model.embed(record["text"])
            record["question_embeddings"] = []
            for question in record["questions"]:
                record["question_embeddings"].append(self.model.embed(question["question"]))
        self.embedding_time = time.process_time() - t

        self.records = self.input

        self.next(self.perform_test)
