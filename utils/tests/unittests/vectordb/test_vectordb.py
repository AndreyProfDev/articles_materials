...
from vectordb.vectordb import VectorDB, VectorIndex


class MockedEmbeddingModel:
    def __init__(
        self, text_to_embedding: dict[str, list[float]], dimension: int
    ) -> None:
        self.text_to_embedding = text_to_embedding
        self.dimension = dimension
        self.model_info = EmbeddingModelInfo(
            model_name="test_model", dimension=dimension, cost_per_mln_tokens=0.1
        )
...
class VectorDBTestCase(unittest.TestCase):
    ...
    def test_initalize_vector_db(self):
        ...
        self.assertEqual(
            vector_db.find_text("Test text 3", top_k=1, index_name="test_index"),
            ["Test text 1"],
        )
    ...
    def test_initalize_vector_db_with_many_indices(self):
        ...
        self.assertEqual(
            vector_db.find_text("Test text 3", top_k=1, index_name="test_index2"),
            ["Test text 3"],
        )
