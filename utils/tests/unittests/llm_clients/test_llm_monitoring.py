...
from llm_clients.schema import ChatMessage, GenericLLMResponse, LLMModelInfo
...
class MockedLLMClient:
    ...
    def chat(
        ...
        return GenericLLMResponse(
            response=response, promt_tokens=2, completion_tokens=3, time_to_generate=0
        )
...
class LLMCostMonitoringTestCase(unittest.TestCase):
    ...
    def test_llm_cost_monitoring(self):
        ...
        client = MockedLLMClient(
            request_to_responce={
                ChatMessage(role="user", content="request"): underlying_response
            }
        )
