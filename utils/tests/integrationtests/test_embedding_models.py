...
from embedding_models.providers import supported_models
from embedding_models.providers.hugging_face import HFEmbeddingModel
from embedding_models.providers.open_ai import OpenAIEmbeddingModel
...
class CalculatingAndStoringEmbeddingsTestCase(unittest.TestCase):
    ...
    def test_populate_database_with_openai_embeddings(self):
        ...
        embedding_model = OpenAIEmbeddingModel(
            api_key=str(api_key), model_info=supported_models.TEXT_EMBEDDING_3_SMALL
        )
        embedding_model = CachedEmbeddingModel(model=embedding_model)
        embedding_model = EmbeddingModelWithMonitoring(model=embedding_model)
    ...
    def test_populate_database_with_hugging_face_embeddings(self):
        ...
        embedding_model = HFEmbeddingModel(
            supported_models.ST_POLISH_PARAPHRASE_FROM_DISTILROBERTA
        )
